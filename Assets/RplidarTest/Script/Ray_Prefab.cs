...
//Manager 센서에서 데이터를 가져오고 그걸 토대로 판단을 해주는게 나을 것 같음
//그래서 현재 무빙 상태다 라고 하면 그제서야 해당 지점에 레이를 쏴주는거 고
//아니면 Holding 상태면 레이를 쏘지 않고
//다음 지점을 찍는데까지 시간이 걸렸는데 다시 제자리다 하면 그 때 다시 여기를 밟았다고 판단을 하는 것


//1. Rplidar에서 센서 데이터 기반 해당 지점에 프리팹 생성
//2. 프리팹에서 현재 위치 데이터 Manager 센서에 전달 O
//3. 매니저 센서에서 현재 가져온 위치정보를 받아오고 현재 상태를 판별함
//4. 판별한걸 기반으로 아까 위치에 공 오브젝트를 만들어냄

//무빙일 경우 해당 좌표에 오브젝트 생성